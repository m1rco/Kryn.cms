...
var DatePicker = new Class({
    ...
    if( dp.options.withTime ){
        ...
        timeHour = new Element('select', {'id': dp.id+'_timeHourSelect', style:'width: 46%'})
          .addEvent('change', function(){ _this.updateValues(dp) } );
        for( var i=0; i< 24; i++){
            var j = (i < 10) ? '0'+i : i;
            new Element('option', {value: j, text: j, selected: (i==dp.hour)}).inject( timeHour );
        }
        timeMinutes = new Element('select', {'id': dp.id+'_timeMinutesSelect', style:'width: 46%'})
          .addEvent('change', function(){ _this.updateValues(dp) } );
        for( var i=0; i< 60; i++){
            var j = (i < 10) ? '0'+i : i;
