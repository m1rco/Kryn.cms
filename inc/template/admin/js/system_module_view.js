var admin_system_module_view = new Class({
    ...
    renderContent: function( pItem ){
        ...
        var td = new Element('td', {html: 
            (pItem.serverVersion)?pItem.serverVersion:'<span style="color: gray;">'+_('Cannot retrieve latest version')+'</span>'
        }).inject( tr );
        ...
        if(! pItem.installed ){
            ...
            .addEvent('click', this.install.bind(this))
            .inject( td );
        } else {
        	
        	if( pItem.name != 'admin' && pItem.name != 'kryn' && pItem.name != 'users' ){
	            new ka.Button(_('Deinstall'))
	            .addEvent('click', this.renderRemove.bind(this))
	            .inject( td );
        	} else {
        		new Element('div', {
    ...
    renderDepends: function( pValues, pInstall ){
        ...
        .addEvent('click', this.loadInfos.bind(this))
        .inject( div );
        
        this.dependsGoNext = new ka.Button(_('Next'))
        .addEvent('click', function(){
    ...
    _renderDepends: function( pRes, pDiv ){
    	...
    	$H(pRes.depends_ext).each(function(extInfo, extKey){
	       	 ...
	       	 if( !extInfo.installed && extInfo.server_version && !extInfo.server_version_not_ok){
		         ...
		         if( !extInfo.installed ){
		        	 ...
		        	 if( extInfo.server_version_not_ok ){
			        	 ...
			        	 new ka.Button(_('Install')).inject(); */
	       	 ...
	       	 new Element('img', {
	       		src: _path+'inc/template/admin/images/icons/'+img+'.png',
    ...
    _checkDepends: function(){
    	...
    	if( go == true ){
    		...
    		this.dependsUl.getElements('li').each(function(li){
    			...
    			var div = new Element('div', {
    				'class': 'ka-admin-modules-depends-status',
    				style: 'padding: 10px;',
    				text: _('Loading ...')
    			}).inject( li );
    ...
    renderInstallForm: function( pValues, pInstall ){
        ...
        if( this.isUpdate ){
            new ka.Button(_('Update now'))
            .addEvent('click', this._install.bind(this, false))
            .inject( div );
        } else {
            new ka.Button(_('Install now'))
            .addEvent('click', this._install.bind(this, false))
            .inject( div );
        }
    ...
    remove: function(){
        ...
        this.win._confirm( _('Really remove ?'), function(p){
            ...
            new Request.JSON({url: _path+'admin/system/module/remove', noCache: 1, onComplete: function(res){
                ka.loadMenu();
                ka.wm.sendSoftReload('admin/system/module');
                this.win.close();
    ...
    _renderRemove: function(){
        ...
        new Request.JSON({url: _path+'admin/system/module/getChangedFiles', noCache: 1, onComplete: function(res){
            ...
            if( res.modifiedFiles.length > 0 ){
                ...
                res.modifiedFiles.each(function(item, key){
                    ...
                    this.removeCheckboxes.include(item, 
                        new Element('input', {
                            type: 'checkbox'
                        }).inject( li, 'top' )
                    ); 
