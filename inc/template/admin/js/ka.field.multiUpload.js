ka.field_multiUpload = new Class ({
	Implements : [Events, Options],
	options : {
		buttontitle :  _('Upload new file( s )'),
    ...
    _uploadStart: function( pFile ){
        ...
        ka.uploads[this.win.id].addFileParam( pFile.id, 'path', this.options.savepath );
        ka.uploads[this.win.id].startUpload( pFile.id );
        ka.fupload.addToUploadMonitor( pFile, ka.uploads[this.win.id] );
        this.fireEvent('start', [ pFile, ka.uploads[this.win.id]] );
    },
    ...
    _addUploadedFileToPanel : function (pFile, pSecParam) {
		...
		new Element('img', { 'src' : _path+'inc/template/admin/images/icons/cross.png' }).inject(delBtn);
