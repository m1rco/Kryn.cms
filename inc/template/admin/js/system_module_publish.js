var admin_system_module_publish = new Class({
    ...
    initialize: function( pWin ){
        ...
        if( ka.settings.system.communityId > 0 ){
            this.uploadBtn = this.topGroup.addButton(_('Upload this version'), _path+'inc/template/admin/images/icons/add.png', this.add.bind(this));
        }
        this.createPkg = this.topGroup.addButton(_('Create package'), _path+'inc/template/admin/images/icons/compress.png', this.getPackage.bind(this));
    ...
    loadInfo: function(){
        ...
        new Request.JSON({url: _path+'admin/system/module/getPublishInfo', noCache: 1, onComplete: function(res){
            this.loader.hide();
            this.info = res;
            this.renderInfo();
    ...
    renderInfo: function(){
        ...
        if( this.info.serverVersion == this.info.config.version &&
        	...
        	(this.module != 'admin' && this.module != 'users' && this.module != 'kryn')
